...
This actually strengthens the QUIC/UDP hypothesis rather than dead-ending it: Docker Desktop for Mac's `vpnkit` user-space networking layer has long-standing, independently-documented UDP/QUIC reliability problems (`docker/for-mac`/`docker/desktop-feedback` issue trackers include reports that UDP problems specifically break HTTP/3 workloads — HTTP/3 being QUIC-over-UDP, the same protocol family any-sync uses) — this is corroborating evidence from an unrelated source, not just this one warning line. No client-side "prefer TCP" flag for `anytype-cli`/`anytype-heart` is documented anywhere findable (checked `any-sync-dockercompose`'s own Configuration wiki, which only covers self-hosted coordinator/node-side transport config, not client-side) — and self-hosting isn't the setup being tested here anyway (this is the default Anytype Network case), so there's no coordinator config to change even if there were a documented knob.

**Next concrete, non-invasive step (not yet run):** test `anytype-cli` running *natively* on the Mac host (outside Docker Desktop's networking layer entirely, via the plain install script) against the same account, to isolate whether Docker Desktop's `vpnkit` is really the culprit. If auto-login succeeds there, that's strong confirmation, and it's also a legitimate fallback deployment path in its own right for just this one service — not something to decide unilaterally here, since it's a real architecture trade-off (this doc's own Docker-vs-`anytype service install` discussion earlier in the project's history) rather than a bug fix.
...
4. **Still open, now the actual blocker:** `docker-compose.anytype-bot.yml`'s account bootstrap succeeds, but the main container's auto-login against that identity fails (`DeadlineExceeded`, then either `signature is invalid` or `application is not running` depending on the run — same root cause, different surface error). Leading hypothesis is Docker Desktop for Mac's `vpnkit` networking layer, which has independently-documented UDP/QUIC reliability problems (Section 10, beta rounds 2-3) — the host-level sysctl experiment couldn't run on this test host (Docker Desktop's VM isn't reachable the normal way, and the change would be shared/host-wide). Next step: test `anytype-cli` natively outside Docker Desktop's networking to isolate the variable (Section 10). This blocks reaching a live Anytype instance at all right now.
