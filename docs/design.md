...
2. `docker compose ... exec anytype-cli anytype auth apikey create hermes-integration` — mints the API key scoped to *this* identity/node. This is `ANYTYPE_API_KEY`; `ANYTYPE_API_BASE_URL` is this container's published port (`http://127.0.0.1:31012` by default).
...
4. `docker compose ... exec anytype-cli anytype space join "<invite-link>"`, then the human approves the join request in-app. Will likely need explicit promotion to Editor role to match the "free writes by default" decision in Section 2 — default post-join role isn't confirmed.
