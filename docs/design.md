...
**Net status:** all other plausible explanations on this host (gluetun, Tailscale exit-node routing, MTU, DNS, Caddy) are now ruled out rather than merely untested, which if anything makes the underlying "Docker Desktop's user-space networking" theory more likely, not less. The vpnkit-backend-switch experiment above is the next thing to try before falling back to running this one service natively.
...
4. **Still open, now the actual blocker:** `docker-compose.anytype-bot.yml`'s account bootstrap succeeds, but the main container's auto-login against that identity fails (`DeadlineExceeded`, then either `signature is invalid` or `application is not running` depending on the run — same root cause, different surface error). A side-by-side native-vs-Docker test confirms Docker Desktop's user-space networking is the actual variable (Section 10, beta round 4) — specifically the **gVisor** network backend on this host, not `vpnkit` as earlier rounds assumed. Other plausible causes on this host (gluetun, Tailscale exit-node routing, MTU, DNS, Caddy) are ruled out, not just untested. Next step, stays fully in Docker: switch Docker Desktop's network backend from `gvisor` to `vpnkit` and retest (exact steps in Section 10) — reversible, no shared-VM access needed. Falling back to running this one service natively remains the last resort, not yet needed. This blocks reaching a live Anytype instance at all right now.
