...
**Still open:** account *creation* is now proven to work in CI, on both architectures, for the first time. The actual `PreferYamuxTransport` auto-login fix — the entire point of this fork — has *still* never been exercised against a real stored account; CI's functional test only covers `CreateWallet`, not `Authenticate()`/`attemptAutoLogin()`. That's the next beta pass. Filing anything upstream (the `PreferYamuxTransport` flag, and now also the discarded-response bug in `CreateWallet`) remains deliberately deferred until the actual fix is confirmed working end-to-end, per explicit instruction.
...
4. **Root cause found and patched; account creation now proven working in CI, auto-login itself still not verified live (beta rounds 5-7, Section 10):** the auto-login failure traces to `anytype-cli` never setting the `PreferYamuxTransport` flag anytype-heart's own code documents as existing "in case client has some problems with QUIC" — confirmed by reading the actual source across `any-sync`, `anytype-heart`, and `anytype-cli`, not inferred from logs. Patched in a fork (`thisispete/anytype-cli`). Getting a working build took three real, distinct bugs, each caught and fixed in turn: (1) dynamic linking against glibc's C++ runtime, missing from the Alpine runtime image; (2) root-owned build output breaking the verification step's `chmod`; (3) a missing `-X core.Version=...` ldflag that let a real upstream bug in `anytype-cli` (a discarded RPC response) crash `AccountCreate` server-side. CI now statically-link-checks, and functionally smoke-tests both `--help` and a real `auth create` against both architectures before ever uploading an artifact — each of bugs 2 and 3 were caught by the pipeline catching up to what a tester had already found once, not novel foresight. Current image (`ghcr.io/thisispete/anytype-cli:patched`) builds, links, and creates accounts successfully — proven in CI, not assumed. **Still open:** the actual `PreferYamuxTransport` auto-login fix — the entire reason for this fork — has never been exercised against a real *stored* account; CI's functional test only covers fresh creation (`CreateWallet`), not `Authenticate()`/`attemptAutoLogin()`. That's the next beta pass. Confirmed done: the ghcr.io package is public (confirmed by the user). File upstream (both the `PreferYamuxTransport` flag and the discarded-response bug found in round 7) once the fix itself is confirmed working. The gVisor/vpnkit backend-switch experiment from round 4 remains superseded — no longer the next step unless the patch turns out not to fully resolve it.
