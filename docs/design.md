...
**Still open:** the image now builds, statically links, and runs correctly enough to pass `--help` in a bare Alpine container — but the actual `PreferYamuxTransport` fix has *still* never been exercised against a real stored account. Passing a smoke test is not the same as the auto-login fix working; that's the next beta pass, for real this time. Also still unverified: whether `ghcr.io/thisispete/anytype-cli` is set to public visibility (confirmed by the user directly, not via the API — the token used to build it lacked the `packages` scope to check itself). Filing this upstream with `anyproto/anytype-cli` remains deliberately deferred until the patch is confirmed working end-to-end, per explicit instruction.
...
4. **Root cause found and patched; image now builds correctly, still not verified live (beta rounds 5-6, Section 10):** the auto-login failure traces to `anytype-cli` never setting the `PreferYamuxTransport` flag anytype-heart's own code documents as existing "in case client has some problems with QUIC" — confirmed by reading the actual source across `any-sync`, `anytype-heart`, and `anytype-cli`, not inferred from logs. Patched in a fork (`thisispete/anytype-cli`). First build attempt shipped a broken binary (dynamically linked against glibc's C++ runtime, missing from the Alpine runtime image) that crashed before ever reaching the fix under test; fixed (static linking + a DNS-resolver flag) and CI now verifies both statically-linked-ness and that the binary actually runs in a bare Alpine container before ever uploading it, catching a second build bug (root-owned output) on its own before it reached a tester. Current image (`ghcr.io/thisispete/anytype-cli:patched`) builds, links, and smoke-tests clean. **Still open:** the actual auto-login fix has never been exercised against a real stored account — that's the next beta pass. Confirmed done: the ghcr.io package is public (confirmed by the user). File upstream once the fix itself is confirmed working. The gVisor/vpnkit backend-switch experiment from round 4 is superseded by this — no longer the next step unless the patch turns out not to fully resolve it.
