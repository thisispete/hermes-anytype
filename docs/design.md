...
4. **Still open, now the actual blocker:** `docker-compose.anytype-bot.yml`'s account bootstrap succeeds, but the main container's auto-login against that identity fails (`DeadlineExceeded` / `signature is invalid` — Section 10, beta round 2). Leading lead is a host-level UDP/QUIC buffer-size ceiling, documented with exact commands in Section 10, unconfirmed. This blocks reaching a live Anytype instance at all right now.
